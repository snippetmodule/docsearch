...
// 加载配置文件
var objConfig = JSON.parse(fs.readFileSync(process.cwd()+"/src/lib/logger/log4j.json", "utf8"));
...
helper.i = function (msg) {
    if (msg == null)
        msg = "";
    logInfo.info.apply(logInfo, arguments);
};

helper.w = function (msg) {
    if (msg == null)
        msg = "";
    logWarn.warn.apply(logWarn, arguments);
};
...
helper.e = function (msg, exp) {
    ...
    if (exp != null)
        msg += "" + exp;
    logErr.error.apply(logErr, arguments);
};
